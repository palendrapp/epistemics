...
The [0.2 diagnostic revision](docs/company-investigation-v2.md) now supplies **12 cases / 48 checkpoints**, guaranteed correction coverage, matched research prices, prospective query forecasts and separate source-feedback/report-adjustment parameters. Humans can complete the same task through a private browser flow; agents use its bound MCP. Two synthetic recovery runs and transport/UI checks pass. Real-agent 0.2 acceptance, human acceptance, repeatability and calibrated uncertainty remain pending; no new passport trait is issued.
