...
The [current delivery strategy](docs/decision-value-pilot.md) targets one portable behavioral claim that improves a concrete buyer decision: whether to require independent verification before accepting a research conclusion. The [sixteen-session repeatability and research-packet comparison](docs/source-panel-results-2026-09-26.md) is complete. It supports reporting scoped repeatability and presentation results, while configuration-specific fits added no transfer benefit. The [clarified source interface](docs/source-delivery-v2.md) and [scoped source passports](docs/source-learning-passport.md) are implemented; the [eight-context assigned-verification comparison](docs/verification-stage-a-results-2026-09-26.md) is complete. Blanket purchasing improved Brier but reduced net payoff in every matched contrast. The [24-context cost-aware follow-up](docs/cost-aware-results-2026-09-26.md) is complete: it reduced check expenditure by 92.375% and beat always-check, but lost to no-check in every matched repeat. Both configurations failed the benefit gate. Conditional sample updating was close to the numerical benchmark; only two distinct companies accounted for all harmful selective revisions. Next audit expected decision value and starting-report calibration over independent worlds before further account-backed collection; passport-guided allocation remains deferred. General support benefit is tested separately from personalization; a bounded design-partner assessment can test demand before automated payment infrastructure is needed.
