...
| Active discovery development | 0.1 six-case agent run; separate 0.2 with 12 balanced/matched cases, prospective research forecasts, conditional source-feedback/report-rate model, recovery checks and shared human/MCP collection | Actual 0.2 agent/human acceptance, repeatability, calibrated uncertainty and a frozen validation design; 0.1 candidate fit remains inadequate |
...
Next run a bounded actual 0.2 agent usability/cost check and a human acceptance pass, then repeat configurations on new cases. Record possible human carryover and the extra burden/effect of prospective questions. Freeze an independent prediction plan before making passport claims; test one targeted-support intervention only after identifying a repeatable difficulty. Neither the synthetic checks nor the original six-case run establishes stable traits, a research-policy parameter or intervention benefit; no large collection is automatically warranted.
