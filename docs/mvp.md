...
| Active discovery development | Completed 0.3 pilot and evidence-linked passports; new eight-case auxiliary diagnostic with shared browser/MCP collection and two-seed synthetic recovery | Actual agent/human diagnostic acceptance; causal-model and broader discovery coverage; parameter explanations still lack adequate empirical new-case fit |
...
The next bounded increment is now implemented: the [auxiliary diagnostic](auxiliary-diagnostic.md) has eight matched cases, conditional forecasts, a neutral repeat and direct resolution, with browser/MCP collection and synthetic recovery. It separates expected associations, selective propagation and report adjustment under a small candidate family; it does not identify causal structure or validate passport traits. Complete a small actual-agent acceptance and human walkthrough before broader collection.

In parallel, [provider enrollment](provider-enrollment.md) now commits identity, configuration and protocol before collection, then rechecks authority and exact artifacts before issuance. The local preview collects a new synthetic core evaluation under that identity and traverses the consumer; synthetic origin correctly remains ineligible. Live acceptance needs an owned devnet identity and a new real-agent evaluation. Hosted paid evaluation jobs remain the next commercial milestone; targeted interventions require their own benefit comparison.

## Delivery order

| Stage | Deliverable | Completion criterion |
| --- | --- | --- |
| 1. Measurement and predictive validation — next evidence milestone | Human acceptance, matched causal/auxiliary contrasts and a costed revision informed by the completed comparisons | On new held-out cases, meet declared prediction criteria before claiming individualized predictive value; separately test assistance benefit after cost |
