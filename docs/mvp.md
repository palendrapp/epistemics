...
| Active discovery development | Four-checkpoint company investigation, joint source/auxiliary inference, research choice, correction, model discrimination and off-grid parameter recovery; bound MCP and readable/private JSON exports | Small fresh-agent usability/cost run, repeatability, human interface acceptance and a frozen validation design; research-policy coverage remains limited |
...
The [literature review](cognitive-modeling-review-2026-09-20.md) identified a gap between the narrow evidence-weighting benchmark and the passport's inference objective. The first [company-investigation module](company-investigation.md) is now implemented separately: source/auxiliary predictions, one research choice and a correction, with simulation checks before real collection. Next run its six-episode development form with a fresh respondent to check comprehension and cost, then repeat with new cases and plan independent validation. Its synthetic results do not establish real-agent traits; source-audit choices are too sparse under the current reference for a research-policy claim.
