...
Default validation uses 32 repetitions per generating profile, or 128 generated datasets per seed. Recovery does not establish robustness to unknown noise, heterogeneous response strategies, lapses, different direct-audit treatment or baseline misspecification. No actual agent or human has completed this new diagnostic yet. Next is a small acceptance run with qualitative review before expanding collection or adding its parameters to passports. All `model_parameters_are_passport_traits` and `empirical_predictive_validation` flags remain false.
