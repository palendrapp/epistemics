...
Expiry is an issuer validity bound, not proof of freshness. Consumers must separately inspect evaluation completion time and current configuration. There is no withdrawal/status resolver yet, so online consumers must not treat this command as proof that an attestation remains unrevoked. The next [agent-economy integration](agent-economy.md) adds identity resolution, lifecycle status and task policy before any automated payment.
