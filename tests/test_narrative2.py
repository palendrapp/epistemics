...
from epistemics.narrative.inference import condition, fit, normalize, predictions
from epistemics.narrative.models import Answer, Joint
from epistemics.narrative.service import fingerprint as narrative_fingerprint
from epistemics.narrative2.battery import background_facts, public_trial, schedule
from epistemics.narrative2.inference import analyze
from epistemics.narrative2.models import Manifest, Observation, Report, Trial
from epistemics.narrative2.service import NarrativeService, create, encoded, export, load
from epistemics.narrative2.simulation import answers, recovery, simulate, synthetic_inputs
from epistemics.narrative2.web import LocalServer
...
def trial_id(a, index):
    return public_trial(a, index, "stop" if index >= 2 else None).trial_id


def test_matched_background_corroboration_and_selective_disclosure():
    ...
    for a in assignments:
        ...
        assert public_trial(a, 0).new_documents[0] == public_trial(pair, 0).new_documents[0]
        assert public_trial(a, 0).new_documents[1] != public_trial(pair, 0).new_documents[1]
        assert "index" not in public_trial(a, 0).model_dump()
        assert public_trial(a, 0).checkpoint == 1
        assert public_trial(a, 3, "stop").checkpoint == 4
        assert 'At "Dashboard arrives"' in public_trial(a, 0).instructions
...
def test_contracts_and_exact_trial_reconstruction(tmp_path):
    ...
    coverage = report.analysis["research_coverage"]
    assert coverage["both_audits_have_at_least_two_informative_cases"]
    assert coverage["both_audits_observed"]
    assert coverage["observed_mean_foregone_information_bits"] == 0
    assert coverage["fixed_policy_mean_foregone_information_bits"]["demand_audit"] > 0
    assert coverage["fixed_policy_mean_foregone_information_bits"]["pipeline_audit"] > 0
    assert all(
        row["research_choice_changed"] for row in report.analysis["matched_corroboration_contrasts"]
    )
...
def test_immutable_branch_choice_concurrent_retry_and_binding(tmp_path):
    ...
    with pytest.raises(ValueError, match="current checkpoint"):
        service.submit(trial_id(a, 3), reports[3])
    with pytest.raises(ValueError, match="signal forecasts"):
        service.submit(trial_id(a, 0), reports[3])
    service.submit(trial_id(a, 0), reports[0])
    with ThreadPoolExecutor(4) as pool:
        receipts = list(
            pool.map(
                lambda _: NarrativeService(tmp_path / "c", a.assignment_id).submit(
                    trial_id(a, 1), reports[1]
                ),
                range(4),
            )
        )
    ...
    with pytest.raises(ValueError, match="cannot be changed"):
        service.submit(trial_id(a, 1), reports[1].model_copy(update={"query": "stop"}))
...
def test_undefined_references_and_fit_exclusions_are_explicit():
    ...
    result = analyze(
        ...
        [
            a.model_copy(update={"corroboration": "demand"}),
            a.model_copy(update={"corroboration": "artifact"}),
        ],
    )
    assert all(
...
def test_schema_snapshots_and_previous_evaluators_unchanged():
    for model, name in [(Manifest, "collection"), (Trial, "trial"), (Report, "report")]:
        ...
        assert Path(f"schemas/narrative-{name}.v2.json").read_bytes() == encoded(schema)
    assert (
        narrative_fingerprint()
        == "89a971dc11790af2e7137ed4ce4ccc42546e4cbc9f97401e83faf147c2ff3fd7"
    )
    assert (
...
def test_actual_mcp_transport_with_research_choice(tmp_path):
    ...
    async def run():
        params = StdioServerParameters(
            ...
            args=["-m", "epistemics.narrative2.mcp_server"],
            env={
                **os.environ,
                "EPISTEMICS_NARRATIVE2": str(tmp_path / "c"),
