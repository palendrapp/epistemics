...
from epistemics.investigation2 import ANALYSIS_VERSION, BATTERY_VERSION, EVALUATOR_VERSION
...
class Trial(Model):
    schema_version: Literal["epistemics.investigation-trial.v2"] = (
        "epistemics.investigation-trial.v2"
    )
    battery_version: Literal["company-investigation/0.2.0"] = BATTERY_VERSION
    trial_id: str
    ...
    def public_stage(self):
        ...
        if self.transcription_checked and self.correction_offset_pct not in (None, 0):
        ...
        if self.expectation_queries != (
            list(MEASUREMENTS) if self.index == 1 and self.transcription_checked else []
        ):
...
class Manifest(Model):
    schema_version: Literal["epistemics.investigation.v2"] = "epistemics.investigation.v2"
    battery_version: Literal["company-investigation/0.2.0"] = BATTERY_VERSION
    evaluator_version: Literal["investigation-evaluator/0.2.0"] = EVALUATOR_VERSION
    analysis_version: Literal["investigation-analysis/0.2.0"] = ANALYSIS_VERSION
    study_id: str
    created_at: str
...
class Report(Model):
    schema_version: Literal["epistemics.investigation-report.v2"] = (
