"""Matched prose/fact renderings; prospective predictions precede the evidence."""

import random
from uuid import UUID

from epistemics.narrative.models import STAGES, Assignment, Trial
...
INSTRUCTIONS = (
    ...
    "interpretation of the documents. At checkpoint 2, report your joint probabilities and choose "
...
def schedule(seed):
    ...
    for domain in ("subscriptions", "distributor"):
        for direction in ("rise", "fall"):
            d, s = truths.pop()
            pair = identifier()
            for presentation in ("narrative", "facts"):
                assignments.append(
                    Assignment(
...
def events(a):
    ...
    return {
        ...
        "signal_event": f"E: the upcoming preliminary dashboard says {quantity} {change} by more "
...
def background_facts(a):
    ...
    return [
        ...
        reporting,
        "The suspected reporting problem has not been confirmed or quantified.",
...
def public_trial(a, index, query=None):
    ...
    if index < 2 and query is not None:
        raise ValueError("No research choice before checkpoint 2")
    d = s = None
    if index == 0:
        facts = background_facts(a)
        documents = [" ".join(facts)] if a.presentation == "narrative" else facts
    elif index == 1:
        documents = [
