...
from epistemics.narrative.models import Answer
from epistemics.narrative.service import NarrativeService


def create_server(directory, assignment_id):
    service = NarrativeService(directory, assignment_id)
    server = FastMCP(
        "Epistemics narrative inference 0.1", instructions=service.describe()["instructions"]
    )
...
if __name__ == "__main__":
    create_server(os.environ["EPISTEMICS_NARRATIVE"], os.environ["EPISTEMICS_ASSIGNMENT"]).run(
        transport="stdio"
    )
