...
from epistemics.narrative.models import Manifest, Report, Trial
from epistemics.narrative.service import create, encoded, export, save
...
def main():
    ...
    run.add_argument("--port", type=int, default=8772)
    ...
    try:
        if args.command == "create":
            ...
            from epistemics.narrative.web import serve
            ...
            for model, name in [
                (Manifest, "narrative-collection.v1"),
                (Trial, "narrative-trial.v1"),
                (Report, "narrative-report.v1"),
            ]:
                schema = model.model_json_schema()
                schema["$schema"] = "https://json-schema.org/draft/2020-12/schema"
                (args.directory / f"{name}.json").write_bytes(encoded(schema))
        else:
            from epistemics.narrative.simulation import recovery
