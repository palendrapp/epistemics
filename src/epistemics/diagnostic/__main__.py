...
from epistemics.diagnostic.models import Manifest, Report, Trial
from epistemics.diagnostic.service import create, encoded, export, save


def main():
    parser = argparse.ArgumentParser(description="Development auxiliary-inference diagnostic")
    ...
    validation.add_argument("--seed", type=int, default=20260923)
    ...
    try:
        if args.command == "create":
            ...
            from epistemics.diagnostic.web import serve

            serve(args.directory, args.port)
        elif args.command == "schema":
            args.directory.mkdir(parents=True, exist_ok=True)
            for model, name in [
                (Manifest, "auxiliary-collection.v1"),
                (Trial, "auxiliary-trial.v1"),
                (Report, "auxiliary-report.v1"),
            ]:
            ...
            print(
                json.dumps({k: v for k, v in result.items() if k != "family_comparisons"}, indent=2)
            )
