...
if (fragment.has("access")) {
  sessionStorage.setItem("source-delivery-access", fragment.get("access"));
  history.replaceState(null, "", location.pathname);
}
const token = sessionStorage.getItem("source-delivery-access") || "";
...
async function render() {
  ...
  if (trial.stage === "research") {
    for (const [q, option] of Object.entries(trial.research_options)) node("p", `${queryNames[q]}: ${option.description}`, form);
