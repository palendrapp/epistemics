...
from epistemics.investigation3.models import MEASUREMENTS, STAGES, Trial
from epistemics.investigation3.world import ASSUMPTIONS

INSTRUCTIONS = (
    "Investigate this fictional company using only its supplied record. Give probabilities "
    "for next-year growth exceeding 12%, one additional historical source report, separate from the displayed archive, understating its "
    "audit by more than 2 points, and this company's backlog exceeding 22. In the API, submit "
    "probabilities from 0 to 1 in increments of 0.01: 0.65 means 65%. A purchased "
...
def public_trial(assignment, world, index, selected_query=None):
    ...
    documents.append(
        {
            ...
            "text": (
                ...
                if checked
                else "The transcription has now been verified. Apply the offset and replacement value in the review document below."
                if index == 3
    ...
    return Trial(
        ...
        transcription_verified_at_start=checked,
        transcription_reviewed=index == 3,
