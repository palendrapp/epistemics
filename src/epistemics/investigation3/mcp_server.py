...
from epistemics.investigation3.models import Answer as InvestigationAnswer
...
def create_server(directory, assignment_id):
    ...
    server = FastMCP(
        "Epistemics investigation 0.2", instructions=service.describe()["instructions"]
    )
...
def main():
    create_server(os.environ["EPISTEMICS_INVESTIGATION3"], os.environ["EPISTEMICS_ASSIGNMENT"]).run(
        transport="stdio"
    )
