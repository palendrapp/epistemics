...
def create_server(directory):
    ...
    server = FastMCP(
        "Epistemics cost-aware verification 0.2", instructions=service.describe()["instructions"]
    )
...
if __name__ == "__main__":
    create_server(os.environ["EPISTEMICS_SOURCE_SELECTIVE"]).run(transport="stdio")
