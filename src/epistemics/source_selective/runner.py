"""Bounded fresh-context acceptance and repeated three-policy comparison; operator-only."""
...
from epistemics.source_selective.analysis import gate, repeatability, repeated_contrast
from epistemics.source_selective.policy import POLICIES, RULE
...
def command(root, entry, config):
    ...
    return [
        s.replace(
            "epistemics.source_panel.mcp_server", "epistemics.source_selective.mcp_server"
        ).replace("EPISTEMICS_SOURCE_PANEL", "EPISTEMICS_SOURCE_SELECTIVE")
        for s in args[:-1]
    ] + [PROMPT]


def balanced_runs(configurations, worlds, seed):
    rng = random.Random(seed)
    base = list(POLICIES)
    rng.shuffle(base)
    if worlds == ["acceptance"]:
        return [
            {
                "run_id": f"{configurations[0]}-acceptance-{p}-1",
    ...
    if len(configurations) != 2 or set(worlds) != {"a", "b"}:
        raise ValueError("Comparison requires two configurations and two worlds")
    blocks = [(w, r) for w in worlds for r in (1, 2)]
    rng.shuffle(blocks)
    orders = ((0, 1, 2), (1, 2, 0), (2, 0, 1), (2, 1, 0))
    runs = []
    for block, (world, repeat) in enumerate(blocks):
        for position in range(3):
            for j, config in enumerate(configurations):
                order = orders[block] if j == 0 else tuple(reversed(orders[block]))
                policy = base[order[position]]
                runs.append(
                    {
                        "run_id": f"{config}-{world}-{policy}-{repeat}",
...
def prepare(root, validation_paths, *, phase, acceptance=None):
    ...
    if phase not in ("acceptance", "comparison"):
    ...
    for path in validation_paths:
        ...
        if (
            ...
            or result["datasets"] < 288
            or set(result["policies"]) != set(POLICIES)
            or not all(p["passed"] for p in result["policies"].values())
        ):
    ...
    if phase == "comparison":
        if acceptance is None:
            raise ValueError("Review acceptance before the repeated comparison")
        ...
        for entry in prior["runs"]:
            ...
            if (
                ...
                or evidence["binding"]["price_seed"] != prior["price_seeds"][entry["world"]]
            ):
    ...
    actual = subprocess.check_output(
        [
            sys.executable,
            "-c",
            "from epistemics.source_selective.service import fingerprint; print(fingerprint())",
        ],
        cwd=root,
    ...
    price_seeds = {w: secrets.randbits(64) for w in worlds}
    ...
    runs = balanced_runs(list(configs), worlds, order_seed)
    plan = {
        "schema_version": "epistemics.source-selective-plan.v1",
        "phase": phase,
        "created_at": now(),
        "implementation_sha256": fingerprint(),
        ...
        "policy_rule": RULE,
        "ordering": "Four randomized world/repeat blocks; each policy occurs once per configuration within each block; rotated and reversed policy orders, two configurations per batch",
        "order_seed": order_seed,
        "runs": runs,
        "limits": {
            "max_attempts_per_run": 1,
            "per_run_seconds": 900,
            "total_seconds": 1800 if phase == "acceptance" else 7200,
            "max_known_processed_tokens": 12000000 if phase == "acceptance" else 40000000,
            "admission_reserve_tokens_per_run": 3000000,
            "concurrency": 2,
        },
        "analysis": {
            "primary": "Cost-aware minus none and cost-aware minus always, mean net realized payoff over twelve later companies; average the two context repeats within each config/world before equal weighting of the two worlds",
            "secondary": [
                "final Brier",
                "gross payoff",
                "verification cost",
            ...
            "feasibility_gate": {
                "cost_aware_vs_none_mean_at_least": 0.01,
                "cost_aware_vs_always_mean_at_least": 0.0,
                "both_worlds_nonnegative_against_each": True,
                "applies_to": "repeat means, separately per configuration",
            },
            "uncertainty": "Two independent worlds per configuration; two fresh contexts per world/policy measure response variability, not new worlds; report repeat differences and signs, no independent-company bootstrap or population claim",
            "trajectory": "Full fresh policy contexts; policies declared upfront; comparison includes anticipation and carryover",
            "acceptance": "Engineering/comprehension only; excluded from the repeated comparison estimates",
    ...
    for e in runs:
        ...
        create(
            ...
            policy=e["policy"],
            seed=seeds[e["world"]],
            price_seed=price_seeds[e["world"]],
        )
    return plan
...
def summarize(root):
    ...
    for e in plan["runs"]:
        ...
        if (
            ...
            or evidence["binding"]["price_seed"] != plan["price_seeds"][e["world"]]
    ...
    if plan["phase"] == "comparison":
        for c in plan["configurations"]:
            comparisons[c], repeats[c] = {}, {}
            for w in plan["world_seeds"]:
                arms = {p: [runs[f"{c}-{w}-{p}-{r}"]["analysis"] for r in (1, 2)] for p in POLICIES}
                comparisons[c][w] = {
                    "cost_aware_vs_none": repeated_contrast(arms["none"], arms["cost_aware"]),
                    "cost_aware_vs_always": repeated_contrast(arms["always"], arms["cost_aware"]),
                    "always_vs_none": repeated_contrast(arms["none"], arms["always"]),
                }
                repeats[c][w] = {p: repeatability(a) for p, a in arms.items()}
    result = {
        "schema_version": "epistemics.source-selective-summary.v1",
        "plan_sha256": digest(raw),
        "phase": plan["phase"],
        "runs": runs,
        "contrasts": comparisons,
        "repeatability": repeats,
        "gates": {c: gate(v) for c, v in comparisons.items()}
        if plan["phase"] == "comparison"
        else None,
...
def main():
    ...
    p.add_argument("--phase", choices=["acceptance", "comparison"], required=True)
