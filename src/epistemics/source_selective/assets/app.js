...
if (fragment.has("access")) {
  sessionStorage.setItem("source-selective-access", fragment.get("access"));
  history.replaceState(null, "", location.pathname);
}
const token = sessionStorage.getItem("source-selective-access") || "";
...
async function render() {
  ...
  if (trial.assigned_verification) { const v = trial.assigned_verification; node("p", `Assigned policy: ${({always: "Always check", none: "No check", cost_aware: "Cost-aware"})[v.policy]}. Available sample price: ${v.available_check_cost.toFixed(3)} points; assigned cost: ${v.charged_cost === null ? "determined after your provisional answer" : v.charged_cost.toFixed(3) + " points"}. ${v.description}`); }
