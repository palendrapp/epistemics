...
from epistemics.narrative2.battery import schedule
from epistemics.narrative2.service import NarrativeService, create, export
...
def synthetic_inputs(assignments, seed):
    ...
    for a in assignments:
        strong, uncertain = rng.uniform(0.82, 0.94), rng.uniform(0.35, 0.6)
        d, s = (strong, uncertain) if a.corroboration == "demand" else (uncertain, strong)
        prior = quantize([d * (1 - s), (1 - d) * s, d * s, (1 - d) * (1 - s)])
        likelihoods = np.round(
            [rng.uniform(0.75, 0.95), rng.uniform(0.6, 0.85), 1.0, rng.uniform(0.02, 0.15)], 2
        )
        posterior = normalize(prior * likelihoods)
        query = (
            "demand_audit"
            if entropy(posterior @ DEMAND) > entropy(posterior @ ARTIFACT)
            else "pipeline_audit"
        )
        inputs.append((prior, likelihoods, query, a))
    return inputs
...
def simulate(directory, seed=20260924):
    manifest = create(
        ...
        {
            "kind": "agent",
            "subject_id": "synthetic:narrative2-reference",
            "configuration": {
                "model": "synthetic",
                "model_version": "1",
    ...
    for entry in synthetic_inputs(manifest.assignments, seed):
        ...
        for answer in answers(entry):
            service.submit(service.get_trial()["trial"]["trial_id"], answer)
        service.finish()
...
def recovery(seed=20260924, repetitions=16):
    ...
    result = {
        ...
        "both_research_branches_in_synthetic_inputs": {e[2] for e in inputs}
        == {"demand_audit", "pipeline_audit"},
    ...
    result["passed"] = bool(
        all(exact)
        and result["both_research_branches_in_synthetic_inputs"]
        and np.max(mae) <= 0.1
