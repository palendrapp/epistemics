...
from epistemics.narrative.models import STAGES, Answer, Query
...
class Assignment(Model):
    ...
    presentation: Literal["narrative"] = "narrative"
    corroboration: Literal["demand", "artifact"]
    demand_outcome: bool
    artifact_outcome: bool


class Trial(Model):
    schema_version: Literal["epistemics.narrative-trial.v2"] = "epistemics.narrative-trial.v2"
    battery_version: Literal["narrative-inference/0.2.0"] = "narrative-inference/0.2.0"
    trial_id: str
    checkpoint: int = Field(ge=1, le=4, strict=True)
    stage: str
    company: str
    instructions: str
    demand_event: str
...
class Manifest(Model):
    schema_version: Literal["epistemics.narrative-collection.v2"] = (
        "epistemics.narrative-collection.v2"
    )
    battery_version: Literal["narrative-inference/0.2.0"] = "narrative-inference/0.2.0"
    evaluator_version: Literal["narrative-evaluator/0.2.0"] = "narrative-evaluator/0.2.0"
    study_id: str
    created_at: str
    ...
    def balanced(self):
        ...
        for domain in ("subscriptions", "distributor"):
            for direction in ("rise", "fall"):
                ...
                if len(group) != 2 or {a.corroboration for a in group} != {"demand", "artifact"}:
                    raise ValueError("Cross domain, direction and corroboration")
...
class Report(Model):
    schema_version: Literal["epistemics.narrative-report.v2"] = "epistemics.narrative-report.v2"
    manifest: Manifest
    ...
    def complete_collection(self):
        from epistemics.narrative2.battery import public_trial
