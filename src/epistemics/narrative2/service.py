...
from epistemics.narrative2.battery import INSTRUCTIONS, public_trial, schedule
from epistemics.narrative2.models import Answer, Manifest, Observation, Report
...
def fingerprint():
    ...
    paths += sorted((root.parent / "narrative").glob("*.py"))
    paths += [
        root.parent / p
        for p in (
            "models.py",
...
def export(directory):
    ...
    report = Report(
        ...
        limitations=[
            ...
            "Matched corroboration cases differ in evidence, not just formatting; stated information values are conditional on respondent reports.",
