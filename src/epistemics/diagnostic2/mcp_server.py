...
from epistemics.diagnostic2.service import DiagnosticService


def create_server(directory, assignment_id):
    service = DiagnosticService(directory, assignment_id)
    server = FastMCP(
        "Epistemics auxiliary diagnostic 0.2", instructions=service.describe()["instructions"]
    )
...
if __name__ == "__main__":
    create_server(os.environ["EPISTEMICS_DIAGNOSTIC2"], os.environ["EPISTEMICS_ASSIGNMENT"]).run(
        transport="stdio"
    )
