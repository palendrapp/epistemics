...
from epistemics.diagnostic2.models import Manifest, Report, Trial
from epistemics.diagnostic2.service import create, encoded, export, save
...
def main():
    ...
    try:
        if args.command == "create":
            ...
            from epistemics.diagnostic2.web import serve

            serve(args.directory, args.port)
        elif args.command == "simulate":
            from epistemics.diagnostic2.simulation import simulate
            ...
            for model, name in [
                (Manifest, "auxiliary-collection.v2"),
                (Trial, "auxiliary-trial.v2"),
                (Report, "auxiliary-report.v2"),
            ]:
                schema = model.model_json_schema()
                schema["$schema"] = "https://json-schema.org/draft/2020-12/schema"
                (args.directory / f"{name}.json").write_bytes(encoded(schema))
        else:
            from epistemics.diagnostic2.simulation import recovery
