"""Recovery under both assigned branches; language/verification benefit is not simulated."""
...
from epistemics.source_verification.presentation import POLICIES
...
def simulate(directory, seed, policy, family="joint_process", gain=1.0):
    manifest = create(directory, PARTICIPANT, seed=seed, policy=policy, synthetic=True)
    service = VerificationService(directory)
    answers = []
    for i in range(36):
        trial = service.get_trial()["trial"]
        answer = synthetic_answer(
            manifest, i, raw_predictions(manifest, i, answers), family, gain, 0, POLICIES[policy]
        )
...
def recovery(seed, repetitions=32):
    ...
    for policy, query in POLICIES.items():
        ...
        results["passed"] = bool(
            min(results["primary_pair_accuracy"].values()) >= GATES["minimum_primary_pair_accuracy"]
    ...
    return {
        "schema_version": "epistemics.source-verification-recovery.v1",
        "seed": seed,
        "repetitions_per_family_per_policy": repetitions,
        "datasets": repetitions * 3 * 2,
