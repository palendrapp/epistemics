"""Assigned actions bound before collection; public answers never include a research choice."""
...
from epistemics.source_verification.analysis import metrics
from epistemics.source_verification.presentation import (
    POLICIES,
    Answer,
    assignments,
    describe,
...
def create(directory, participant, *, policy, seed=None, synthetic=False):
    ...
    binding = {
        "schema_version": "epistemics.source-verification-binding.v1",
        "version": VERSION,
        "base_manifest_sha256": base.manifest_hash,
        "implementation_sha256": fingerprint(),
        "created_at": now(),
        "policy": policy,
        "assignments": assignments(manifest, policy),
...
def verify_evidence(report, evidence):
    ...
    policy = b["policy"]
    if (
        evidence["schema_version"] != "epistemics.source-verification-evidence.v1"
        or b["schema_version"] != "epistemics.source-verification-binding.v1"
        ...
        or b["assignments"] != assignments(report.manifest, policy)
        or b["description"]
        != describe(
            {
        ...
        or evidence["analysis"] != metrics(report, policy)
    ):
    ...
    for i, (o, lock) in enumerate(zip(report.observations, evidence["locks"], strict=True)):
        expected_query = POLICIES[policy] if o.trial.stage == "research" else None
        public = present(o.trial, policy)
...
class VerificationService:
    ...
    def get_trial(self):
        ...
        with self.base.state() as state:
            ...
            if trial is not None:
                public = present(trial, self.binding["policy"])
                i = len(state["answers"])
    ...
    def get_history(self):
        ...
        for row in result["history"]:
            row["trial"] = present(Trial.model_validate(row["trial"]), self.binding["policy"])
        ...
        return {
            **self.base.finish(),
            "completion_questions": [
                "Who determined whether you received an independent check?",
    ...
    def evidence(self):
        ...
        with self.base.state() as state:
            result = {
                "schema_version": "epistemics.source-verification-evidence.v1",
                ...
                "analysis": metrics(report, self.binding["policy"]),
            }
        ...
        lines = [
            ...
            *[
            ...
            "The canonical report is a private calculation ledger with injected policy actions. Use this evidence together with that report; its legacy research-choice labels do not describe the participant's choice.",
